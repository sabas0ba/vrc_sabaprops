...
タコには `OctopusDrift` と `Octopus` の体の動作を使用します。頭頂部を進行方向へ向け、腕を曲げて絞る屈伸で加速し、その後は惰性で進みます。`beatFrequency` が屈伸の周期を決め、既定は 0.65 Hz（約 1.54 秒周期）です。上下を含む大きな経路の変化は、既定では平均 12～30 秒の経由点間隔で滑らかに続きます。
