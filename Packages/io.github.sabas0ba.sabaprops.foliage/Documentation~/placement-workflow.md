...
Surface Vine / Rhizome Patch は、複製後の再生成で他オブジェクトのメッシュを上書きしないよう、
所有者と共有参照を確認して必要な場合に別アセットを作ります。未保存シーンでは永続的な所有者を
確定できないため、再生成ごとに新しいアセットを作ります。継続して調整する場合は先にシーンを保存してください。
既存アセットは他シーンから参照される可能性があるため、自動削除しません。
