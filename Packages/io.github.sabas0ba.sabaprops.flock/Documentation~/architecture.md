...
| UV6 | 動作中の体の余白 x (m) | 余白 y (m) | 余白 z (m) | 予約 |
...
これらの群れでは個体の中心が範囲の壁から体長 1 つ分内側に収まります。Wander、FreeFlight、Jet、Float、OctopusDrift、FloorGlide は独立した軌道です。動作と大きさのばらつきを含む High Mesh の到達半径を UV6 に焼き込み、範囲から一度だけ差し引きます。個体が入る十分な寸法の水槽では、旋回しても体が壁を越えないようにします。旧 Mesh の UV6 が空の場合は体長を余白に使います。変更後は群れを再生成してください。

個体の向きは、`t ± 0.05 s` の位置の差 (中心差分) から求めます。傾きは同じ 3 点から求めた横方向の加速度に比例させます。上昇角には上限があり、魚が真上を向くことはありません。

周回する動き (Thermal、BaitBall、Tornado) では、次のように速度を制限しています。

- 群れの中心の移動速度を、周回の接線速度の 3 割以下に抑える。中心の移動が個体自身の運動を打ち消して向きが反転するのを防ぐため。
- 周回の角速度を 2.5 rad/s 以下に抑える。

Wander / FreeFlight は個体の seed から旋回方向と位相を選び、半径・中心・高度・角速度を異なる低周波で変化させます。細長い範囲では縦横比に応じて角速度を抑えます。Jet はこの経路の時刻を収縮の位相で変調し、前進速度を基準の 0.35～1.65 倍で変えます。Float は seed 付き経由点の三次 B スプラインです。既定速度では経由点を 120～300 秒ごとに切り替え、位置と速度を連続に保ちます。クラゲの傘は短周期で脈動し、移動中も直立します。衝突回避は行いません。

## 体の動作

OctopusDrift は三次元の経由点を巡り、頭頂部である Mesh の +Y 軸を移動方向へ向けます。腕を曲げて後方へ絞る屈伸を推進力のパルスとし、周期の 0.18 倍を時定数とする一次の減衰応答を速度へ適用します。屈伸後も惰性で進み、次の屈伸まで速度が滑らかに低下します。この応答を解析的に積分した時計で経路を評価するため、フレーム間の状態や runtime script は不要です。通常は約 1.54 秒周期で屈伸し、大きな経路変化の経由点間隔は平均 12～30 秒です。

FloorGlide は Wander と同じ水平経路・向き・傾きを使用し、高さを `0.6 × Wander の高さ − 0.3 × 移動空間の半高さ` に変換します。水平方向の自由度を保ち、下部を泳ぐ傾向を強めます。壁際での上昇や腹側を見せる専用の旋回はありません。Collider、地形、プレイヤーへの衝突判定はありません。
