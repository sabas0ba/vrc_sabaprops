...
タコには `OctopusDrift` と `Octopus` の体の動作を使用します。クラゲと同じ滑らかな三次元の経由点を約 10 倍速く巡り、既定では 12～30 秒ごとに経由点が変わります。向きは移動方向から独立し、腕は外套の脈動と別に動きます。

マンタには `FloorGlide` を使用します。範囲の下部を主に巡り、水平経路の端で上昇して腹側を外へ向けながら旋回します。GameObject を水槽内の配置空間の中心に置き、内寸から余白を引いた半寸法を指定してください。壁は `settings.area` の境界として表現し、実際の Collider や凹凸を検出する機能はありません。
...
`Wander` / `FreeFlight` / `Jet` / `Float` / `OctopusDrift` / `FloorGlide` の範囲は、体を含む配置空間の半寸法です。水槽では内寸の半分からガラスや底砂の余白を引いて指定してください。Shader が動作中の体の到達半径をさらに一度だけ差し引きます。範囲の各軸は到達半径より大きくする必要があります。UV6 を追加した版へ更新した際には、既存の群れを再生成してください。
