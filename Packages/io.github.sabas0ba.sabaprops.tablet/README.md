...
各操作ページ、設定画面、11 テーマの画像は [操作・設定・テーマの画像ガイド](Documentation~/setup-guide.md) に掲載しています。
...
配置は一辺 3 m の立方体の 4 側面と天井です。各面は 3 m × 3 m で、側面と天井が隙間なく接します。
側面は内向きで高さ 0〜3 m、天井は高さ 3 m です。
