...
まばらな鳥には `FreeFlight` を選び、翼開長に対して十分に広い範囲と少ない個体数を指定してください。イカ・タコには `Jet` と同名の体の動作を組み合わせると、収縮と加速が連動します。クラゲには `Float` を使用します。既定速度では経由点の間隔が 120～300 秒の浮遊です。速度倍率を上げると長周期の移動を早送りして確認できます。

`Wander` / `FreeFlight` / `Jet` / `Float` の範囲は、体を含む配置空間の半寸法です。水槽では内寸の半分からガラスや底砂の余白を引いて指定してください。Shader が動作中の体の到達半径をさらに一度だけ差し引きます。範囲の各軸は到達半径より大きくする必要があります。UV6 を追加した版へ更新した際には、既存の群れを再生成してください。
