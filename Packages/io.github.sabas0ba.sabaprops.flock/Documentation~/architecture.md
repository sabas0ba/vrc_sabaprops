...
OctopusDrift は Float の経由点の時計を 10 倍にし、既定では 12～30 秒の間隔で滑らかに漂います。姿勢の旋回と三次元の腕の動作は、移動方向から独立しています。イカの Jet とは異なり、移動速度を外套の収縮に固定していません。

FloorGlide は配置範囲から動作中の体の半径を引いた空間で楕円経路を巡ります。通常は下部を泳ぎ、水平経路の四つの端に近づくと上昇します。旋回時には腹側が範囲の外を向くように最大約 83° 傾けます。これは水槽の境界を使った時刻ベースの演出で、Collider、地形、プレイヤーへの衝突判定はありません。
