...
[樹木の調整・確認ガイド](Documentation~/tree-authoring.md)を参照してください。
...
- `Crown Envelope Strength` は樹冠外形に応じて枝の成長量を短縮する強さです。`0` は自由成長、`1` は外形制約を適用します。枝は付け根から方向を保って短縮され、短縮した枝からの子枝も短くなります。主幹先端より上にも枝が育つ空間を設けています
- 外形補正後は縦横比と枝の角度を保つ等方スケールで大きさを補正します。X/Z方向だけの拡大は行いません
- `Crown Width Scale` は外形判定に使う基準半径です。大きくすると横方向へ枝が伸びる余地が増えます。最終的な高さ・幅には `Crown Volume Scale` によるスケールも影響します
- `Crown Volume Scale` は自由成長Meshの**外接箱体積**を `1` としたサイズ補正目標です。葉の密度や枝葉の実占有体積ではありません。後述の半径拡大前の形状から補正量を求めるため、半径を広げても自動で縮小されません。最終的な見栄えはUnityで確認してください
- `Crown Radial Scale` は一次枝の付け根を基準に、枝の中心線と葉の配置位置を水平方向へ広げます。実在種presetの既定値は `1.5` です。幹の形状、枝の太さ、葉の大きさ、中心線の高さは維持します。`1` に戻すと半径拡大を無効にできます。最終体積を旧Sampleの±20%に抑える制約は適用しません
- `Primary Branch Departure` は一次枝の出始めの向きを、幹の接線から `Branch Angle` の方向へ寄せる割合です。実在種presetの既定値は `0.25` です。`0` は幹に沿って立ち上がり、大きくすると付け根から外向きに分岐します
...
- 樹皮色は幹・枝・端面・葉柄で共通の高さに基づいて補間し、分岐次数による接合部の色の急変を防ぎます。葉は互生・対生・輪生・2 針束を分けます
...
| Preset | 構造 | 葉・樹皮 |
|---|---|---|
| Japanese Zelkova / ケヤキ | 街路樹向けの丸い樹冠、上向きの螺旋枝 | 互生する小型広葉、灰褐色から若枝色 |
